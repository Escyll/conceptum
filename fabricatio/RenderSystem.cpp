...
void RenderSystem::progress(float /*timeDelta*/)
{
    ...
    auto lightEntities = m_registry.view<PointLight, Transform>();
    const auto& [lightEntityPointLight, lightEntityTransform] = m_registry.get<PointLight, Transform>(lightEntities.front());

    PositionedPointLight pointLight{lightEntityPointLight, lightEntityTransform.transformation()};
    ...
    for (const auto& [entity, mesh, transform] : view.each())
    {
        ...
        drawMesh(mesh, model, view, projection, lights);
    }
}
